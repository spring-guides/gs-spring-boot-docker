...
To push to a Docker registry you use the `docker` goal, instead of `dockerbuild`.
To do that you will need to have permission to push to Dockerhub. You won't have permission to push to `springio` by default, so change the image prefix to your own Dockerhub ID, and `docker login` to make sure you are authenticated before you run `mvn`.
