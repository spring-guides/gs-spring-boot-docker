...
You also need https://docker.com[Docker], see https://docs.docker.com/installation/#installation for details on setting Docker up for your machine. Before proceeding further, verify you can run `docker` commands from the shell.
...
----
./gradlew build && java -jar build/libs/{project_id}-0.0.1-SNAPSHOT.jar
----
====
...
[bash,subs="attributes"]
----
./mvnw package && java -jar target/{project_id}-0.0.1-SNAPSHOT.jar
----
====
