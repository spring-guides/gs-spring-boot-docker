...
This Dockerfile is very simple, but that's all you need to run a Spring Boot app with no frills: just Java and a JAR file. The build will `COPY` the project JAR file into the container as "app.jar" and then it is executed in the `ENTRYPOINT`.
