...
[NOTE]
====
The order of plugins matters. If you run deploy, but the container fails due to no manifest file found for your JAR, make sure that the docker plug in appears below the spring-boot-maven-plugin. If you don't explicitly declare the spring-boot-maven-plugin, add
```
<plugin>
