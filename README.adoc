...
In the Maven `pom.xml` you should add a new plugin like this (see https://github.com/spotify/dockerfile-maven#example[the plugin documentation] for more options):
:
...
* The image tag, usually latest or the artifact id.
...
You can build a tagged docker image using:

----
$ ./mvnw package
----

It can be pushed to a remote repository using:
----
$ ./mvnw deploy
----

NOTE: You don't have to push your newly minted Docker image to actually run it. Moreover the "push" will fail if you aren't a member of the "springio" organization on Dockerhub. Change the build configuration and the command line to your own username instead of "springio" to make it actually work.
