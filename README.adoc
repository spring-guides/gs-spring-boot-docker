...
Instead of building with the Docker command line, you might want to use a build plugin. Google has an open source tool called https://github.com/GoogleContainerTools/jib[Jib] that has Maven and Gradle plugins. Probably the most interesting thing about it is that you don't need docker. In fact you don't even need a `Dockerfile`. It builds the image using the same standard output as you get from `docker build` but doesn't use `docker` unless you ask it to - so it works in environments where docker is not installed (not uncommon in build servers). 

NOTE: The images generated by Jib by default run as root. Check the https://github.com/GoogleContainerTools/jib/tree/master/jib-maven-plugin#configuration[configuration guide] for how to change that and other default settings.

=== Build a Docker Image with Maven
To get started quickly, you can run Jib without even changing your `pom.xml` (and remember the `Dockerfile` if it is still there is ignored):

```
$ ./mvnw com.google.cloud.tools:jib-maven-plugin:dockerBuild -Dimage=springio/gs-spring-boot-docker
```

You can add `-Djib.container.user=user:group` if you want to avoid running as root. You also have to set `-Djib.from.image` to a base image that has those user accounts (the default base image is `gcr.io/distroless` which only has a `root` user).
...
As with the Maven build, you can add `-Djib.container.user=user:group` if you want to avoid running as root, as long as you also set `-Djib.from.image` to a base image that has those user accounts. And there is also a build task to build and push to the Docker registry:
